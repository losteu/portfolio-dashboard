...
function render() {
  ...
  const hasSalesScenario = !!(UI.offG.size || UI.offC.size || UI.offP.size || UI.selG || UI.selC.size);
  ...
  const changed = UI.offA.size>0 || UI.offG.size>0 || UI.offC.size>0 || UI.offP.size>0 || hasSel;
  const disN = UI.offA.size + UI.offG.size + UI.offC.size + UI.offP.size;
  ...
  const summaryUsesAreaSheet = D.areaRows.length > 0 && (!hasSalesScenario || !salesScenarioAffectsSales);
  const baseAreaSheet = summaryUsesAreaSheet ? calcAreaSheetSummary(false) : null;
  const curAreaSheet = summaryUsesAreaSheet ? calcAreaSheetSummary(true) : null;
  const baseS = summaryUsesAreaSheet
    ? {...base, areaResult:baseAreaSheet.areaResult, needNormUp:baseAreaSheet.needNormUp, opResult:base.cm1 + baseAreaSheet.areaResult, finalResult:base.cm1 + baseAreaSheet.areaResult - base.holdingTotal, mArea:base.rev>0?baseAreaSheet.areaResult/base.rev:0, mOp:base.rev>0?(base.cm1 + baseAreaSheet.areaResult)/base.rev:0, mFinal:base.rev>0?(base.cm1 + baseAreaSheet.areaResult - base.holdingTotal)/base.rev:0}
    : {...base, needNormUp:base.needNormUp||0};
  const curS = summaryUsesAreaSheet
    ? {...cur, areaResult:curAreaSheet.areaResult, needNormUp:curAreaSheet.needNormUp, opResult:cur.cm1 + curAreaSheet.areaResult, finalResult:cur.cm1 + curAreaSheet.areaResult - cur.holdingTotal, mArea:cur.rev>0?curAreaSheet.areaResult/cur.rev:0, mOp:cur.rev>0?(cur.cm1 + curAreaSheet.areaResult)/cur.rev:0, mFinal:cur.rev>0?(cur.cm1 + curAreaSheet.areaResult - cur.holdingTotal)/cur.rev:0}
    : {...cur, needNormUp:cur.needNormUp||0};
  const areaReconGap = summaryUsesAreaSheet ? curS.areaResult - cur.areaResult : 0;
  ...
  function buildProductStats(salesRows, allocRows) {
    ...
    salesRows.forEach(s => {
      ...
      if(!gpSalesMap.has(k)) gpSalesMap.set(k,{rev:0,qty:0,clients:new Set()});
    ...
    let rows=[...tpMap.values()].map(t=>{
      const productKey=getProductKey({gpCode:t.gpCode,gpName:t.gpName});
      const sales=gpSalesMap.get(t.gpCode)||{rev:0,qty:0,clients:new Set()};
      const fin=t.marginInc-t.direct-t.distL-t.distNL;
      const holding=(gpHoldingMap.get(String(t.gpCode))||{holding:0}).holding;
      const productGroup=productGroupMap.get(productKey)||'';
      const off=!!((productKey && UI.offP.has(productKey)) || UI.offA.has(t.group) || (productGroup && eog.has(productGroup)));
      return {...t,productKey,productGroup,off,rev:sales.rev,sqlQty:sales.qty,clientCnt:sales.clients.size,clientList:[...sales.clients].slice(0,5).join(', '),fin,needNormUp:Math.max(-fin,0),holding,finalAfterHolding:fin-holding,margin:t.marginInc>0?fin/t.marginInc:0};
    });
    if(UI.searchTP) rows=rows.filter(t=>t.gpName.toLowerCase().includes(UI.searchTP.toLowerCase())||t.group.toLowerCase().includes(UI.searchTP.toLowerCase()));
  ...
  function buildClientProductStats(rows) {
    const cpMap = new Map();
    rows.forEach(s => {
      const key=s.client+'|||'+s.gpId;
      if(!cpMap.has(key)) cpMap.set(key,{client:s.client,gpId:s.gpId,gpName:s.gpName,gpGroup:s.gpGroup,qty:0,revenue:0,rawCost:0,normCost:0,transport:0,allocDirect:0,allocDistL:0,allocDistNL:0,allocAreaResult:0,normGap:0,allocHolding:0,tpMatchedCnt:0,rowCnt:0,docs:new Map()});
  ...
  const heroCards = [
    ...
    kpi('Рез. участков',fmt(curS.areaResult)+' ₽',`${summaryUsesAreaSheet?'Лист участков':'Связанные продажи'} · ${pct(curS.mArea)}`,changed?curS.areaResult-baseS.areaResult:undefined,curS.areaResult<0),
    holdingConfigured
  ...
  // Hero summary
  html+=`<section class="hero fu"><div class="hero-main${primaryValue<0?' warn':''}"><div class="hero-eyebrow">Главный итог</div><h2>${primaryLabel}</h2><div class="hero-value">${fmt(primaryValue)} ₽</div><p class="hero-sub">${primarySub}</p><div class="hero-foot">${deltaPill(changed?impactDelta:undefined)}<span class="hero-pill">Источник участков: ${summaryUsesAreaSheet?'лист':'аллокация продаж'}</span>${holdingConfigured?`<span class="hero-pill">Холдинг: ${fmtF(holdingConfig.monthly)}/мес</span>`:''}</div></div><div class="hero-grid">${heroCards.join('')}</div></section>`;
  ...
  if(summaryUsesAreaSheet && Math.abs(areaReconGap) > 1) html+=`<div class="rnote">В обзоре финрез участков берется напрямую с листа «Данные по участкам»: ${fmtF(curS.areaResult)}. По связанным продажам сейчас разнесено ${fmtF(cur.areaResult)}, разница ${fmtF(areaReconGap)} пока не попадает в детальные вкладки из-за неполной связки ГП → участки.</div>`;
  if(!summaryUsesAreaSheet && salesScenarioAffectsSales) html+=`<div class="rnote">При фильтрах по клиентам, группам или продукции обзорный финрез участков считается по связанным продажам, а не напрямую по листу участков.</div>`;
  ...
  if(UI.scOpen){
    ...
    html+=`<div class="sc-panel wide"><details${UI._exclOpen?' open':''}><summary class="slb" style="cursor:pointer;user-select:none">Исключения ${UI.offA.size+UI.offG.size+UI.offC.size+UI.offP.size?'<span style="color:var(--red);font-weight:700">'+(UI.offA.size+UI.offG.size+UI.offC.size+UI.offP.size)+' откл.</span>':''}</summary><div style="margin-top:8px;font-size:12px;color:var(--tx3)">Цветной чип = участвует, красный зачеркнутый = исключен. Быстрое меню по названию в таблицах тоже использует этот же сценарий.</div><div style="margin-top:12px;display:flex;flex-direction:column;gap:10px">`;
  ...
  if(UI.tab==='groups'){
    ...
    const gView=UI.grpNegOnly?tabGStats.filter(g=>g[groupResultKey]<0):tabGStats;
  ...
  if(UI.tab==='clients'){
    ...
    const clTotal=tabCStats.length, clPages=Math.max(1,Math.ceil(clTotal/PAGE_SZ)), clPage=Math.min(UI.cliPage,clPages-1);
    const clSlice=tabCStats.slice(clPage*PAGE_SZ,(clPage+1)*PAGE_SZ);
    html+=`<div class="cd fu"><div class="cdh"><h3>Клиенты</h3><p>${clTotal} клиентов${disN?` · исключенные остаются в списке`:''}</p></div><div class="cds"><table><thead><tr>${clCols.map(([k,l])=>`<th>${sortButton('data-clsort',k,l,UI.cliSort,UI.cliSortDir)}</th>`).join('')}</tr></thead><tbody>${clSlice.map(c=>salesMeta.hasSplitCosts
  ...
  if(UI.tab==='clientprod'){
    ...
    const cpNegItems=cpStats.filter(c=>c[cpResultKey]<0);
    const cpWorst10=cpNegItems.slice().sort((a,b)=>a[cpResultKey]-b[cpResultKey]).slice(0,10);
    const cpListNegItems=tabCpStats.filter(c=>c[cpResultKey]<0);
    const cpNegTotal=cpListNegItems.reduce((a,c)=>a+c[cpResultKey],0);
    const cpView=UI.cpNegOnly?cpListNegItems:tabCpStats;
